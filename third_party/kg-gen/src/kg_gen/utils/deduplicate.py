...
class DeduplicateList:
    ...
    def deduplicate(self, items: list[str]) -> list[str]:
        ...
        # semhash has changed fields across versions:
        # older results expose `deduplicated`/`duplicates`, newer expose `selected`/`filtered`.
        selected = getattr(deduplication_result, "selected", None)
        if selected is None:
            selected = getattr(deduplication_result, "deduplicated", None)
        if selected is None:
            raise AttributeError(
                "semhash DeduplicationResult is missing both `selected` and `deduplicated`."
            )

        duplicate_groups = getattr(deduplication_result, "duplicates", None)
        if duplicate_groups is None:
            duplicate_groups = getattr(deduplication_result, "filtered", [])
        if duplicate_groups is None:
            duplicate_groups = []

        self.deduplicated = list(selected)
        self.deduplicated_items = len(self.deduplicated)
        self.duplicate_items = len(duplicate_groups)
        self.reduction = (
        ...
        for duplicate in duplicate_groups:
            original = getattr(duplicate, "record", None)
            duplicate_candidates = getattr(duplicate, "duplicates", None)

            if not original or not duplicate_candidates:
                continue

            first_candidate = duplicate_candidates[0]
            duplicate_value = (
                first_candidate[0]
                if isinstance(first_candidate, (tuple, list)) and len(first_candidate) > 0
                else first_candidate
            )

            if not isinstance(duplicate_value, str):
                continue
            if original not in self.items_map or duplicate_value not in self.items_map:
                continue
